...
class DockerRunner:
    ...
    def _build_strace_cmd(self, command: list[str]) -> tuple[str, list[str]]:
        ...
        strace_cmd = [
            "sh", "-c",
            f"strace -f -ttt -e trace=network -s 256 -o {container_strace_path} -- {escaped_cmd} && sync"
        ]
    ...
    def _run_with_docker_sdk(
        ...
        try:
            ...
            # Check container logs for errors (kept for diagnostics)
            _ = container.logs().decode('utf-8', errors='replace')
    ...
    def _run_with_subprocess(
        ...
        try:
            ...
            # Get container logs for better error messages (ignored content)
            subprocess.run(["docker", "logs", container_id], capture_output=True, text=True, check=False)
