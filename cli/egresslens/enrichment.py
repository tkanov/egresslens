"""Domain enrichment: attribute domains to captured destination IPs.

Used by the backend on upload and by `egresslens check` against on-disk
artifacts. Passive DNS reads the answers already in the trace; the reverse-DNS
fallback makes live queries, so the two surfaces default it differently.
"""
