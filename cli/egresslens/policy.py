...
The engine is shared by both surfaces: the backend runs it at upload time, and
`egresslens check` runs it against on-disk capture artifacts as a CI gate. Either
way the domains a `domain` rule matches come from `egresslens.enrichment`, so a
verdict depends on the artifacts and the enrichment settings, not on which
surface computed it.
...
from egresslens.enrichment import choose_primary_domain, event_domain_candidates
...
def resolve_destinations(
    ...
    Mirrors the per-destination resolution in the backend's compute_aggregates so
    the verdict is consistent with the top-destinations table, but covers *every*
    destination rather than the displayed top 50. Each destination carries a ``domain``
